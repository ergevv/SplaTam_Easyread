...
// 使用 obtain 函数为 geom 的各个成员变量分配内存并填充数据。
// 每个 obtain 调用都会更新 chunk 指针，使其指向下一个未使用的内存位置。
// 第三个参数是所需元素的数量，第四个参数 128 是对齐要求。
CudaRasterizer::GeometryState CudaRasterizer::GeometryState::fromChunk(char*& chunk, size_t P)
{
	...
	cub::DeviceScan::InclusiveSum(nullptr, geom.scan_size, geom.tiles_touched, geom.tiles_touched, P);  //计算tiles_touched的前缀和到tiles_touched

...
def initialize_first_timestep(dataset, num_frames, scene_radius_depth_ratio, mean_sq_dist_method, densify_dataset=None):
    ...
    color, depth, intrinsics = dataset[0]

    pose = torch.eye(4, dtype=torch.float32).to("cuda")
...
def rgbd_slam(config: dict):
    ...
    # 
    dataset = MyBikeDataset(dataset_config["basedir"], os.path.basename(dataset_config["sequence"]), dataset_config["gradslam_data_cfg"],10)
    ...
    for time_idx in tqdm(range(checkpoint_time_idx, num_frames)): # 循环迭代处理 RGB-D 帧，循环的起始索引是 checkpoint_time_idx（也就是是否从某帧开始，一般都是0开始），终止索引是 num_frames
        ...
        color, depth, _ = dataset[time_idx] # 从数据集 dataset 中加载 RGB-D 帧的颜色、深度、姿态等信息
...
# 假设我们想传递 '/path/to/experiment.yaml' 作为 experiment 参数的值
    manual_args = ['configs/my_bike/splatam.py']
